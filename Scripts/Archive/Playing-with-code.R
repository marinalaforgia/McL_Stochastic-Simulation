...
A.bad.DA <- matrix(c(5,5,0.9,0.1), 2, byrow = T) # row 1 - lambda, row 2, germination, col 1 grass, col 2 - high dormant forb, need to include another row for effect of forb on grass and affect of grass on forb
A.bad.DT <- matrix(c(5,5,0.9,0.5), 2, byrow = T) # row 1 - lambda, row 2, germination, col 1 grass, col 2 - low dormant forb
A.good.DA <- matrix(c(30, 30,0.9,0.6), 2, byrow = T)
A.good.DT <- matrix(c(30, 30,0.9,0.8), 2, byrow = T)
#A.inter <- matrix(c(17.5, 17.5, 0.9, 0.7), 2, byrow = T)

# Put matrices together in array
A.DT = array(0,dim = c(2,2,2)) #2 2x2 matrices
A.DT[,,1] = A.bad.DT
A.DT[,,2] = A.good.DT

A.DA = array(0,dim = c(2,2,2)) #2 2x2 matrices
A.DA[,,1] = A.bad.DA
A.DA[,,2] = A.good.DA
...
df = 0.1
dg = 0.7
afg = 2 # this will change, need to include this in the matrix
agf = 0.5
c = 1
N0 = c(2,2)
tf = 10
...
eq <- Annual(tf = 100, N0 = N0, p = 0.5, q = 0.5, df = df, dg = dg, afg = afg, agf = agf, c = c, A = A.DT)
# plot
...
eq <- Annual(tf = 100, N0 = N0,  p = 1, q = 0, df = df, dg = dg, afg = afg, agf = agf, c = c, A = A.DT) 
# plot
...
eq <- Annual(tf = 100, N0 = N0,  p = 0, q = 1, df = df, dg = dg, afg = afg, agf = agf, c = c, A = A.DT) 
# plot
...
eq <- Annual(tf = 100, N0 = N0, p = 0.5, q = 0.5, df = df, dg = dg, afg = afg, agf = agf, c = c, A = A.DA)
# plot
matplot(eq[[2]],type="l",ylim = c(0,35), xlab = "Years", ylab = "Density", main="Germinated and Ungerminated Fraction")
points(eq[[1]]+33, pch = ifelse(eq[[1]]+33 > 34, "+", "-"))
legend(0,33, legend = c("Grass","Forb"), col = c("red","black"), lty = c(2,1),lwd = 1, bty = "n")
...
par(mfrow=c(1,3))
# constantly favorable year
eq <- Annual(tf = 100, N0 = N0,  p = 1, q = 0, df = df, dg = dg, afg = afg, agf = agf, c = c, A = A.DA) 
# plot
matplot(eq[[2]],type="l",ylim = c(0,35), xlab = "Years", ylab = "Density", main="Constantly Favorable Environment")
legend("topleft", legend = c("Grass","Forb"), col = c("red","black"), lty = c(2,1),lwd = 1, bty = "n")

# constantly unfavorable year
eq <- Annual(tf = 100, N0 = N0,  p = 0, q = 1, df = df, dg = dg, afg = afg, agf = agf, c = c, A = A.DA) 
# plot
matplot(eq[[2]],type="l",ylim = c(0,35), xlab = "Years", ylab = "Density", main="Constantly Unfavorable Environment")
legend("topleft", legend = c("Grass","Forb"), col = c("red","black"), lty = c(2,1),lwd = 1, bty = "n")
