...
A.bad <- matrix(c(5,5,0.9,0.1), 2, byrow = T) # row 1 - lambda, row 2, germination, col 1 grass, col 2 forb
A.good <- matrix(c(30, 30,0.9,0.7), 2, byrow = T)
A.inter <- matrix(c(17.5, 17.5, 0.9, 0.7), 2, byrow = T)

# Put matrices together in array
A = array(0,dim = c(2,2,2)) #2 2x2 matrices
A[,,1] = A.bad
A[,,2] = A.good
...
par(mfrow=c(2,1))
# equal probability of good and bad years (germ and ungerm fraction)
eq <- Annual(tf = 50, N0 = N0, p = 0.5, q = 0.5, df = df, dg = dg, afg = afg, agf = agf, c = c, A = A)
# plot
matplot(eq[[2]][,1],type="l",ylim = c(0,23), xlab = "Years", ylab = "Density", main="Germinated and Ungerminated Fraction", lwd = 3)
points(eq[[1]]+20, pch = ifelse(eq[[1]]+20 > 21, "+", "-"))
legend(0,33, legend = c("Grass","Forb"), col = c("red","black"), lty = c(2,1),lwd = 3, bty = "n")
...
matplot(N.germ[,1],type="l",ylim = c(0,25), xlab = "Years", ylab = "Density", main="Germinated Fraction Only", lwd = 2)
points(eq[[1]]+20, pch = ifelse(eq[[1]]+20 > 21, "+", "-"))
legend(0,33, legend = c("Grass","Forb"), col = c("red","black"), lty = c(2,1),lwd = 2, bty = "n")

par(mfrow=c(1,3))
# constantly favorable year
eq <- Annual(tf = 100, N0 = N0,  p = 1, q = 0, df = df, dg = dg, afg = afg, agf = agf, c = c, A = A) 
# plot
...
eq <- Annual(tf = 100, N0 = N0,  p = 0, q = 1, df = df, dg = dg, afg = afg, agf = agf, c = c, A = A) 
# plot
